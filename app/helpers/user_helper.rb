module UserHelper
  def check_occurence(id)
    ...
    if Friendship.occurence_in_friendship(id, current_user.id).count.positive?
      ...
      html += link_to('Accept Friendship', friendship_path(accept_params), id: 'accept', method: :put)
      reject_params = { id: friend[0], user_id: current_user.id, friend_id: id, status: true }
      html += link_to('Reject friendship', friendship_path(reject_params), id: 'reject', method: :delete)
      ...
      html += link_to('Invite', friendships_path(user_id: current_user.id, friend_id: id), id: 'invite', method: :post)
    end
    html.html_safe
  end
end
