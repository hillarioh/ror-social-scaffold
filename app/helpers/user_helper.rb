module UserHelper
  def check_occurence(id)
    ...
    if Friendship.occurence_in_friendship(id, current_user.id).count.positive?
      ...
      html += button_to('Accept Friendship', friendship_path(accept_params), method: :put)      
      reject_params = { id: friend[0], user_id: current_user.id, friend_id: id, status: true }
      html += button_to('Reject friendship', friendship_path(reject_params), method: :delete)
      ...
      html += button_to('Invite', friendships_path(user_id: current_user.id, friend_id: id), method: :post)
    end
    html.html_safe
  end
end
